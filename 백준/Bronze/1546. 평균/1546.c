...
#include <stdio.h>
int main(){
	int total_subj;	//총 과목 개수
	scanf("%d", &total_subj);
	
	double score[total_subj]; //점수 입력받을 배열
	double best_score; //최고점수 기록용
	for(int i=0; i<total_subj; i++){ //점수 기록&최고점수 정하기
		scanf("%lf", &score[i]);
  ...
  double total_sum=0;
	...
	printf("%.2f", total_sum/total_subj);
	return 0;
}
